...
class DiscordRateHonoringSender(client: DiscordClient) {
  ...
  implicit val backpressureStrategy = rest.BackPressureStrategy.Retry(3)
  ...
  private def mention(user: User) = s"<@!${user.id.snowflakeString}>"
  private def mention(channel: headache.Channel) = s"<#${channel.id.snowflakeString}>"

  def send(channel: headache.Channel, msg: String): Future[Unit] = send(channel.id, msg)
